...
const ProductDetailsDialog = ({ open, setOpen, productDetails }) => {
  ...
  const handleAddToCart = () => {
    ...
    const inventory = productDetails.inventory.find(item => item.size === selectedSize);
              ...
              {productDetails?.images.map((img, index) => (
                <button
                  ...
                  className={`flex-shrink-0 w-20 h-20 rounded-md overflow-hidden border-2 transition-all ${
                    selectedImage === index ? 'border-rose-200' : 'border-transparent hover:border-rose-100'
                  }`}
                >
                  <img
                    src={img}
            ...
            <div className="mb-8">
                ...
                <div className="flex">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <StarIcon
                      key={star}
                      className={`w-4 h-4 ${
                        star <= averageRating ? 'text-yellow-400 fill-current' : 'text-neutral-300'
                      }`}
                    />
                  ))}
                </div>
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-2">
                  <Label className="text-sm font-medium text-neutral-700">Select Size</Label>
                  <Button
                    variant="ghost"
                ...
                <div className="flex gap-2 flex-wrap">
                  {Object.keys(productDetails?.price || {}).map((size) => {
                    const inventory = productDetails.inventory.find(item => item.size === size);
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-neutral-700">Chest (in)</TableHead>
                        <TableHead className="text-neutral-700">Waist (in)</TableHead>
                        <TableHead className="text-neutral-700">Hip (in)</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
              ...
              <div className="space-y-6">
                <h2 className="text-xl font-bold text-neutral-800">Customer Reviews</h2>
                      ...
                      <div key={index} className="bg-neutral-50 rounded-lg p-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-neutral-600">{review.reviewMessage}</p>
                          </div>
                        </div>
                      </div>
                    ))
